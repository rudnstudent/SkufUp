...
class SkufUpApp:
    ...
    def listen_loop(self):
        ...
        """
        try:
            # Импортируем детектор (он тяжёлый, поэтому импорт здесь)
    ...
    def launch_target(self):
        ...
        ВАЖНО: Есть cooldown 1 час!
        Это значит, что после запуска игры следующий час
        новые открытия пива не будут ничего запускать.
        
        Зачем это нужно:
        - Чтобы не открывать игру 10 раз подряд если шумно
        - Чтобы можно было открыть вторую банку не запуская игру снова
        """
        import time
        ...
        # Проверяем cooldown: прошёл ли час с последнего запуска?
        # 3600 секунд = 60 минут = 1 час
        if current_time - self.last_launch_time < 3600:
            return  # Ещё не прошёл час - ничего не делаем
        
        # Запускаем в зависимости от типа цели
        if target_type == "game" and target_path:
            try:
                # Запускаем .exe файл
                # cwd=... нужен чтобы игра запустилась в своей папке

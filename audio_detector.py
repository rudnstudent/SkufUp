"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                     Детектор звука открытия банки пива 🍺                    ║
║                                                                              ║
║  Этот модуль отвечает за распознавание характерного звука "пшик" банки.     ║
║                                                                              ║
║  Как работает распознавание:                                                 ║
║  1. Микрофон постоянно записывает звук                                       ║
║  2. Когда слышим громкий звук - анализируем его                             ║
║  3. Сравниваем с эталонными записями открытия банки                         ║
║  4. Если похожесть > 45% - это пиво!                                        ║
║                                                                              ║
║  Эталоны (5 дистанций):                                                      ║
║  - very_close: прямо у микрофона                                            ║
║  - close: 30-50 см                                                          ║
║  - medium: ~1 метр                                                          ║
║  - far: ~2 метра                                                            ║
║  - very_far: 3+ метра                                                       ║
║                                                                              ║
║  Используем лучший результат из всех сравнений.                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""

import numpy as np      # Работа с массивами чисел (математика)
import pyaudio          # Запись звука с микрофона
import time             # Работа со временем (cooldown)
from collections import deque  # Очередь для буфера аудио
from scipy import signal      # Обработка сигналов

# Импортируем настройки и эталонные звуки
from config import AUDIO_SETTINGS, DETECTOR_SETTINGS
from beer_sound_template import get_all_templates, get_template, TEMPLATE_SAMPLE_RATE


class BeerCanDetector:
    """
    Детектор звука открытия банки пива.
    
    Принцип работы:
    1. Слушаем микрофон
    2. При громком звуке - извлекаем "отпечаток" звука (характеристики)
    3. Сравниваем отпечаток с эталонными записями
    4. Если похоже - возвращаем True
    
    Характеристики звука (что анализируем):
    - Огибающая (envelope) - как меняется громкость во времени
    - Спектр (spectrum) - какие частоты присутствуют
    - Позиция пика - где самый громкий момент
    - Скорость нарастания (attack) - как быстро стало громко
    - Скорость затухания (decay) - как быстро стало тихо
    
    Звук открытия банки характерен:
    - Очень быстрым нарастанием (щелчок)
    - Быстрым затуханием
    - Характерными частотами "пшика"
    """
    
    def __init__(self):
        """
        Инициализация детектора.
        
        Загружаем настройки и эталонные звуки.
        """
        # PyAudio - библиотека для работы с аудио
        self.audio = pyaudio.PyAudio()
        self.stream = None      # Аудио поток (пока не открыт)
        self.is_running = False # Работаем ли сейчас
        
        # ===== НАСТРОЙКИ АУДИО =====
        self.rate = AUDIO_SETTINGS["rate"]         # 44100 Hz - частота дискретизации
        self.chunk = AUDIO_SETTINGS["chunk"]       # 2048 - размер буфера
        self.channels = AUDIO_SETTINGS["channels"] # 1 - моно
        
        # ===== НАСТРОЙКИ ДЕТЕКТОРА =====
        self.similarity_threshold = DETECTOR_SETTINGS["similarity_threshold"]  # 0.55 (55%)
        self.peak_threshold = DETECTOR_SETTINGS["peak_threshold"]              # 0.1
        self.cooldown = DETECTOR_SETTINGS["cooldown"]                          # 3 сек
        self.debug_mode = DETECTOR_SETTINGS.get("debug_mode", False)
        
        # ===== ЗАГРУЖАЕМ ВСЕ 5 ЭТАЛОННЫХ ЗВУКОВ =====
        # Загружаем шаблоны с разных дистанций
        self.templates = get_all_templates()
        
        # Извлекаем характеристики всех эталонов (делаем это один раз при запуске)
        self.template_features = {}
        for name, template in self.templates.items():
            self.template_features[name] = self._extract_features(template)
        
        print(f"   📦 Загружено {len(self.templates)} эталонных звуков")
        
        # ===== СОСТОЯНИЕ =====
        self.last_trigger_time = 0  # Время последнего срабатывания
        self.last_debug_time = 0    # Для отладочных сообщений
        
        # Буфер для накопления аудио (500мс = 0.5 сек)
        # Нужен чтобы захватить весь звук открытия целиком
        self.audio_buffer = deque(maxlen=int(self.rate * 0.5))
        
        # Фоновый уровень шума (адаптивный)
        self.background_levels = deque(maxlen=30)
        self.background_level = 0.01
    
    
    # ========================================================================
    # ИЗВЛЕЧЕНИЕ ХАРАКТЕРИСТИК ЗВУКА
    # ========================================================================
    
    def _extract_features(self, audio_data: np.ndarray) -> dict:
        """
        Извлечь характеристики (отпечаток) звука.
        
        Аргументы:
            audio_data: массив numpy с аудио данными
        
        Возвращает:
            dict с характеристиками:
            - envelope: огибающая сигнала (как меняется громкость)
            - spectrum: частотный спектр (какие частоты есть)
            - peak_position: где находится пик (0.0 - начало, 1.0 - конец)
            - attack_rate: скорость нарастания
            - decay_rate: скорость затухания
            - duration: длительность в секундах
        """
        
        # ----- ОГИБАЮЩАЯ (ENVELOPE) -----
        # Берём абсолютные значения (громкость без знака)
        envelope = np.abs(audio_data)
        
        # Сглаживаем огибающую (убираем шум)
        window_size = min(100, len(envelope) // 4)
        if window_size > 0:
            # Скользящее среднее
            envelope_smooth = np.convolve(envelope, np.ones(window_size)/window_size, mode='same')
        else:
            envelope_smooth = envelope
        
        # Нормализуем (приводим к диапазону 0-1)
        max_env = np.max(envelope_smooth)
        if max_env > 0:
            envelope_normalized = envelope_smooth / max_env
        else:
            envelope_normalized = envelope_smooth
        
        # ----- СПЕКТР (ЧАСТОТЫ) -----
        # FFT (быстрое преобразование Фурье) - показывает какие частоты есть в звуке
        fft = np.abs(np.fft.rfft(audio_data))
        fft_normalized = fft / (np.max(fft) + 0.0001)  # +0.0001 чтобы не делить на 0
        
        # ----- ПОЗИЦИЯ ПИКА -----
        # Где находится самый громкий момент (от 0 до 1)
        peak_pos = np.argmax(np.abs(audio_data)) / len(audio_data)
        
        # ----- СКОРОСТЬ НАРАСТАНИЯ (ATTACK) -----
        # Как быстро звук достиг максимума
        peak_idx = np.argmax(np.abs(audio_data))
        if peak_idx > 0:
            attack_rate = np.max(np.abs(audio_data)) / peak_idx
        else:
            attack_rate = 0
        
        # ----- СКОРОСТЬ ЗАТУХАНИЯ (DECAY) -----
        # Как быстро звук стих после пика
        after_peak = np.abs(audio_data[peak_idx:])
        if len(after_peak) > 1:
            decay_rate = (after_peak[0] - after_peak[-1]) / len(after_peak) if after_peak[0] > after_peak[-1] else 0
        else:
            decay_rate = 0
        
        return {
            "envelope": envelope_normalized,
            "spectrum": fft_normalized,
            "peak_position": peak_pos,
            "attack_rate": attack_rate,
            "decay_rate": decay_rate,
            "duration": len(audio_data) / self.rate
        }
    
    
    def _find_sound_segment(self, audio_data: np.ndarray) -> np.ndarray:
        """
        Найти реальные границы звука в буфере.
        
        Использует порог 5% от пика для определения начала и конца звука.
        Возвращает сегмент 100-500мс с реальным звуком (без лишней тишины).
        
        Аргументы:
            audio_data: буфер аудио данных
            
        Возвращает:
            numpy массив с извлечённым сегментом звука
        """
        # Огибающая для поиска границ
        envelope = np.abs(audio_data)
        
        # Сглаживаем
        window = int(self.rate * 0.005)  # 5мс окно
        if window > 0:
            envelope = np.convolve(envelope, np.ones(window)/window, mode='same')
        
        peak_val = np.max(envelope)
        threshold = peak_val * 0.05  # 5% от пика
        
        # Находим где звук начинается и заканчивается
        above_threshold = np.where(envelope > threshold)[0]
        
        if len(above_threshold) < 100:  # Слишком мало данных
            # Fallback: берём окно вокруг пика
            peak_idx = np.argmax(np.abs(audio_data))
            samples_before = int(self.rate * 0.05)
            samples_after = int(self.rate * 0.25)
            start = max(0, peak_idx - samples_before)
            end = min(len(audio_data), peak_idx + samples_after)
            return audio_data[start:end]
        
        start_idx = above_threshold[0]
        end_idx = above_threshold[-1]
        
        # Добавляем небольшие отступы (10мс до и после)
        padding = int(self.rate * 0.01)
        start_idx = max(0, start_idx - padding)
        end_idx = min(len(audio_data), end_idx + padding)
        
        # Ограничиваем размер сегмента (100-500мс)
        min_samples = int(self.rate * 0.1)  # 100мс
        max_samples = int(self.rate * 0.5)  # 500мс
        
        segment_len = end_idx - start_idx
        
        if segment_len < min_samples:
            # Добавляем данные вокруг
            center = (start_idx + end_idx) // 2
            start_idx = max(0, center - min_samples // 2)
            end_idx = min(len(audio_data), start_idx + min_samples)
        elif segment_len > max_samples:
            # Обрезаем до 500мс, центрируя на пике
            peak_idx = start_idx + np.argmax(np.abs(audio_data[start_idx:end_idx]))
            start_idx = max(0, peak_idx - max_samples // 3)
            end_idx = min(len(audio_data), start_idx + max_samples)
        
        return audio_data[start_idx:end_idx]
    
    
    # ========================================================================
    # СРАВНЕНИЕ С ЭТАЛОНАМИ
    # ========================================================================
    
    def _compare_with_single_template(self, features: dict, template_features: dict) -> float:
        """
        Сравнить характеристики звука с одним эталоном.
        
        Аргументы:
            features: характеристики анализируемого звука
            template_features: характеристики эталона
        
        Возвращает:
            float от 0.0 до 1.0 - насколько похоже (1.0 = идеально)
        """
        
        # ----- 1. СРАВНЕНИЕ ОГИБАЮЩИХ (35% веса) -----
        # Приводим к одной длине и считаем корреляцию
        target_len = min(len(features["envelope"]), len(template_features["envelope"]))
        
        # Интерполяция (растягиваем/сжимаем до одной длины)
        env1 = np.interp(
            np.linspace(0, 1, target_len), 
            np.linspace(0, 1, len(features["envelope"])), 
            features["envelope"]
        )
        env2 = np.interp(
            np.linspace(0, 1, target_len), 
            np.linspace(0, 1, len(template_features["envelope"])), 
            template_features["envelope"]
        )
        
        # Корреляция (насколько формы похожи)
        correlation = np.corrcoef(env1, env2)[0, 1]
        if np.isnan(correlation):
            correlation = 0
        envelope_similarity = max(0, correlation)
        
        # ----- 2. СРАВНЕНИЕ СПЕКТРОВ (35% веса) -----
        spec_len = min(len(features["spectrum"]), len(template_features["spectrum"]))
        spec1 = features["spectrum"][:spec_len]
        spec2 = template_features["spectrum"][:spec_len]
        
        spec_correlation = np.corrcoef(spec1, spec2)[0, 1]
        if np.isnan(spec_correlation):
            spec_correlation = 0
        spectrum_similarity = max(0, spec_correlation)
        
        # ----- 3. СРАВНЕНИЕ ПОЗИЦИИ ПИКА (15% веса) -----
        # Чем ближе позиции пиков - тем лучше
        peak_diff = abs(features["peak_position"] - template_features["peak_position"])
        peak_similarity = max(0, 1 - peak_diff * 3)
        
        # ----- 4. СРАВНЕНИЕ СКОРОСТИ НАРАСТАНИЯ (15% веса) -----
        attack_ratio = min(features["attack_rate"], template_features["attack_rate"]) / \
                       (max(features["attack_rate"], template_features["attack_rate"]) + 0.0001)
        
        # ----- ИТОГОВАЯ ПОХОЖЕСТЬ -----
        # Взвешенная сумма всех метрик
        total_similarity = (
            envelope_similarity * 0.35 +   # Форма громкости
            spectrum_similarity * 0.35 +   # Частотный состав
            peak_similarity * 0.15 +       # Позиция пика
            attack_ratio * 0.15            # Скорость атаки
        )
        
        return total_similarity
    
    def _compare_with_template(self, audio_data: np.ndarray) -> tuple:
        """
        Сравнить звук со ВСЕМИ 5 эталонами и вернуть лучший результат.
        
        5 эталонов с разных дистанций:
        - very_close: прямо у микрофона
        - close: 30-50 см
        - medium: ~1 метр
        - far: ~2 метра
        - very_far: 3+ метра
        
        Аргументы:
            audio_data: массив с аудио данными
        
        Возвращает:
            tuple (похожесть, название_эталона)
        """
        # Извлекаем характеристики входного звука
        features = self._extract_features(audio_data)
        
        best_similarity = 0.0
        best_template = ""
        
        # Сравниваем с каждым эталоном
        for name, template_features in self.template_features.items():
            similarity = self._compare_with_single_template(features, template_features)
            if similarity > best_similarity:
                best_similarity = similarity
                best_template = name
        
        return best_similarity, best_template
    
    
    # ========================================================================
    # РАБОТА С МИКРОФОНОМ
    # ========================================================================
    
    def start_stream(self):
        """
        Запустить запись с микрофона.
        
        Открываем аудио поток с параметрами:
        - 16 бит (качество звука)
        - Моно (один канал)
        - 44100 Hz (частота дискретизации)
        """
        self.stream = self.audio.open(
            format=pyaudio.paInt16,          # 16-битный звук
            channels=self.channels,           # 1 = моно
            rate=self.rate,                   # 44100 Hz
            input=True,                       # Это вход (микрофон), не выход
            frames_per_buffer=self.chunk      # Размер буфера
        )
        self.is_running = True
        print("🎤 Микрофон активирован. Ищу звук открытия банки...")
    
    def stop_stream(self):
        """
        Остановить запись с микрофона.
        
        Закрываем поток и освобождаем ресурсы.
        """
        self.is_running = False
        if self.stream:
            self.stream.stop_stream()
            self.stream.close()
        self.audio.terminate()
        print("🔇 Микрофон отключен.")
    
    def read_audio_chunk(self) -> np.ndarray:
        """
        Прочитать порцию аудио данных с микрофона.
        
        Возвращает:
            numpy массив с аудио данными (int16)
        """
        try:
            # Читаем данные из потока
            data = self.stream.read(self.chunk, exception_on_overflow=False)
            # Преобразуем байты в числа
            return np.frombuffer(data, dtype=np.int16)
        except Exception as e:
            print(f"⚠️ Ошибка чтения аудио: {e}")
            return np.zeros(self.chunk, dtype=np.int16)
    
    
    # ========================================================================
    # ГЛАВНЫЙ МЕТОД ДЕТЕКЦИИ
    # ========================================================================
    
    def detect(self) -> bool:
        """
        Проверить, был ли звук открытия банки.
        
        Этот метод вызывается в цикле из gui_app.py.
        
        Алгоритм:
        1. Проверяем cooldown (3 сек между срабатываниями)
        2. Читаем аудио с микрофона
        3. Если тихо - обновляем фоновый уровень
        4. Если громко - анализируем звук
        5. Сравниваем с эталоном
        6. Если похоже > 55% - возвращаем True
        
        Возвращает:
            True  - обнаружен звук открытия банки
            False - не обнаружен (или cooldown)
        """
        current_time = time.time()
        
        # ----- COOLDOWN -----
        # Не срабатываем чаще чем раз в 3 секунды
        if current_time - self.last_trigger_time < self.cooldown:
            return False
        
        # ----- ЧИТАЕМ АУДИО -----
        audio_data = self.read_audio_chunk()
        
        # Нормализуем (приводим к диапазону -1.0 ... +1.0)
        audio_normalized = audio_data / 32768.0  # 32768 = максимум для int16
        
        # Добавляем в буфер (накапливаем 300мс)
        self.audio_buffer.extend(audio_normalized)
        
        # ----- ВЫЧИСЛЯЕМ УРОВНИ -----
        # RMS (Root Mean Square) - средняя громкость
        rms = np.sqrt(np.mean(audio_normalized ** 2))
        
        # Peak - максимальная громкость
        peak = np.max(np.abs(audio_normalized))
        
        # ----- АДАПТИВНЫЙ ФОНОВЫЙ УРОВЕНЬ -----
        # Если тихо - запоминаем как "фон"
        if rms < 0.02:
            self.background_levels.append(rms)
            if len(self.background_levels) > 5:
                self.background_level = np.mean(self.background_levels)
        
        # ----- ОТЛАДОЧНЫЙ ВЫВОД -----
        if self.debug_mode and current_time - self.last_debug_time >= 0.5:
            self.last_debug_time = current_time
            # Рисуем полоску громкости
            bar_length = int(rms * 200)
            bar = "█" * min(bar_length, 50) + "░" * (50 - min(bar_length, 50))
            status = "🔊" if peak > self.peak_threshold else "🔈"
            print(f"{status} [{bar}] RMS:{rms:.3f} Peak:{peak:.3f}", end="\r")
        
        # ----- АНАЛИЗ ГРОМКИХ ЗВУКОВ -----
        # Только если пик выше порога - анализируем
        if peak > self.peak_threshold:
            
            # Берём весь буфер для анализа
            buffer_array = np.array(self.audio_buffer)
            
            # Находим границы звука динамически (как при записи шаблонов)
            segment = self._find_sound_segment(buffer_array)
            
            # Проверяем, достаточно ли данных для анализа
            if len(segment) > 1000:
                
                # СРАВНИВАЕМ СО ВСЕМИ 5 ЭТАЛОНАМИ!
                similarity, matched_template = self._compare_with_template(segment)
                
                if self.debug_mode:
                    print(f"\n   📊 Похожесть: {similarity:.1%} (эталон: {matched_template})")
                
                # Если похожесть выше порога - это пиво!
                if similarity >= self.similarity_threshold:
                    print(f"\n\n🍺 БАНКА ОТКРЫТА! (похожесть: {similarity:.1%}, дистанция: {matched_template})")
                    self.last_trigger_time = current_time
                    self.audio_buffer.clear()  # Очищаем буфер
                    return True
                    
                elif similarity > 0.3:
                    # Похоже, но не достаточно
                    if self.debug_mode:
                        print(f"   ❌ Не похоже на открытие банки ({similarity:.1%})")
        
        return False
